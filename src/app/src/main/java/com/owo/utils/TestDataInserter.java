...
public class TestDataInserter {
    ...
    /** Credentials for the seeded administrator account. Also documented in the README. */
    public static final String ADMIN_EMAIL = "admin@owo.id";
    public static final String ADMIN_PASSWORD = "admin1234";
    public static final String ADMIN_NAMA = "Admin OwO";
    ...
    private static void insertAdminAccount() throws SQLException {
        System.out.println("Inserting admin account...");

        if (AkunDAO.getAkunByEmail(ADMIN_EMAIL) != null) {
            System.out.println("Admin account already present, skipping.");
            return;
        }

        AkunDAO.createAkun(ADMIN_NAMA, ADMIN_EMAIL, ADMIN_PASSWORD, true);
    ...
    private static void seedFlight(float harga, String flightNumber, String origin, String destination,
            String maskapai, String kelas, LocalDateTime waktuKeberangkatan, int kapasitas)
            throws SQLException {
        if (TiketDAO.flightExists(flightNumber)) {
            return;
        }
        TiketDAO.createTiketPesawat(harga, true, flightNumber, origin, destination,
                maskapai, kelas, waktuKeberangkatan, kapasitas);
    }
    ...
    private static void seedHotel(float harga, LocalDate checkIn, LocalDate checkOut,
            String hotelName, String roomNumber, String address, int kapasitas) throws SQLException {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.err.println("Error inserting sample data (" + e.getClass().getSimpleName() + "): "
                    + e.getMessage());
            System.exit(1);
        }
    }
